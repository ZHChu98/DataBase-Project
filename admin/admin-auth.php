...
if (isset($_POST["search-meth"]) && isset($_POST["search-info"])) {
  ...
  $sql = "SELECT DISTINCT * FROM ydzc_auth WHERE $method like '%$keyword%'";
        ...
        <form action="admin-auth.php" method="post">
          <select class="bootstrap-select" data-style="btn-info" name="search-meth" >
            <optgroup label="Picnic">
              ...
              <option value="concat(auth_fname, ' ', auth_lname)">authorName</option>
            </optgroup>
          </select>
  ...
  <div class="container">
    <div class="row">
      <table class="table table-striped">
        <?php
        echo "<tr><td>author ID</td><td>first name</td><td>last name</td><td>email</td><td>house number</td><td>street</td><td>city</td><td>state</td><td>country</td><td>zip</td></tr>";
        $row = mysqli_fetch_assoc($result);
        while($row) {
          $userid=$row['auth_id'];
          echo "<tr><td>{$row['auth_id']}</td><td>{$row['auth_fname']}</td><td>{$row['auth_lname']}</td><td>{$row['auth_email']}</td><td>{$row['auth_hno']}</td><td>{$row['auth_st']}</td><td>{$row['auth_city']}</td><td>{$row['auth_state']}</td><td>{$row['auth_country']}</td><td>{$row['auth_zip']}</td></tr>";
          echo "<td><form action='admin-auth-edit.php' method='post'><button type='submit' class='btn btn-primary' name='userid' value='$userid'>edit</button></form></td>";
          echo "<td><form id='authdeleteform' action='admin-auth-delete.php' method='post'><input type='hidden' name='deleteauthid' value='$userid' ><button type='button' onclick='deleteconfirm()' class='btn btn-primary' name='userid'>delete</button></form></td></tr>";
          $row = mysqli_fetch_assoc($result);
        }
        ?>
      </table>
