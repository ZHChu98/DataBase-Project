...
if (isset($_POST["search-meth"]) && isset($_POST["search-info"])) {
  ...
  $sql = "SELECT DISTINCT * FROM ydzc_reg WHERE $method like '%$keyword%'";
...
<body>
  ...
  <div class="container">
    <div class="row">
        <form action="admin-cust-event.php" method="post">
          <select class="bootstrap-select" data-style="btn-info" name="search-meth" >
            <optgroup label="Picnic">
              <option value="reg_id">register id</option>
  ...
  <div class="container">
    <div class="row">
      <table class="table table-striped">
        <?php
        echo "<tr><td>register ID</td><td>event id</td><td>customer id</td></tr>";
        $row = mysqli_fetch_assoc($result);
        while($row) {
          $userid=$row['reg_id'];
          echo "<tr><td>{$row['reg_id']}</td><td>{$row['evt_id']}</td><td>{$row['cust_id']}</td></tr>";
          $row = mysqli_fetch_assoc($result);
        }
        ?>
      </table>
